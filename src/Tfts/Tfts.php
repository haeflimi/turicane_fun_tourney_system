...
class Tfts {
  ...
  /**
   * Get all groups created for TFTS.
   * 
  ...
  /**
   * Tries to create a group with the given name and adds the given user as
   * member of it.
   *
   * @param string $name
   * @param int $user_id
   ...
   */
  public function createGroup(string $name, int $user_id) {
    $this->verifySystemIsActive();
    if (strlen($name) < 3) {
      throw new Exception('Group name must consist of at least 2 characters!');
    }
    if (is_object(Group::getByName($name))) {
      throw new Exception('Group with name ' . $name . ' already exists!');
    }
    $package = Package::getByHandle('turicane_fun_tourney_system');
    $parentGroup = Group::getByName($package->getPackageName());
    $group = Group::add($name, t('Group generated by TFTS'), $parentGroup, $package);
    $user = User::getByUserID($user_id);
   ...
   * @param int $group_id
   * @param int $user_id
   * @throws Exception If the user is already in group.
   */
  public function joinGroup(int $group_id, int $user_id) {
    $this->verifySystemIsActive();
    $group = Group::getById($group_id);
    $user = User::getByUserID($user_id);
    if ($user->inGroup($group)) {
  ...
  /**
   * Removes the given user from the given group. If the user was the last
   * member of the group, the group is removed as well.
   *
   * @param int $group_id
   * @param int $user_id
   */
